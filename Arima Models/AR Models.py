...
AR_p = int(input("What p do you want for the AR model: "))
...
print(f"Autoregressive Coefficients: {model.coef_}")

test["Predicted Return"] = model.predict(X_test)


plt.plot(test.index, y_test, "o", label="Actual Returns")
plt.plot(test.index, test["Predicted Return"], "x", label="Predicted Returns")

plt.xlabel("Date")
plt.ylabel("Return")
plt.title("AR Model: Actual vs Predicted Returns")
plt.legend()
plt.show()
