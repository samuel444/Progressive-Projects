...
def continuous_evaluation(actual, predicted):
    ...
    if (
        len(actual) < 2
        or len(np.unique(actual)) < 2
        or len(np.unique(predicted)) < 2
    ):
        rank_ic = np.nan
...
def binary_evaluation(
    ...
    if len(classes) != 2:
        raise ValueError(
            f"Binary evaluation expected 2 classes, got: {classes}"
        )
    ...
    if probabilities.ndim == 2:
        ...
        if probabilities.shape[1] != 2:
            raise ValueError(
                "Binary probability matrix must have two columns."
            )
...
def fit_binary_baseline(
    ...
    if len(classes) != 2:
        raise ValueError(
            f"Binary baseline expected 2 classes, got: {classes}"
        )
...
def fit_l2_logistic_regression(
    x_train,
    y_train,
    x_validation,
    y_validation,
...
def fit_l1_logistic_regression(
    x_train,
    y_train,
    x_validation,
    y_validation,
...
def fit_ordinal_regression(
    ...
    raise NotImplementedError(
        "Ordinal regression has no implementation added yet."
    )