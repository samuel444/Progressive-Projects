...
def momentum_score(
    df, columns=("Momentum 20", "Momentum 60", "Momentum 120", "Momentum 252"), weights=None
):
...
def trend_score(
    df, columns=("Trend Strength 20", "Trend Strength 60", "Trend Efficiency 20"), weights=None
):
...
def all_composite_features(df):
    available_momentum = [
        column
        for column in ("Momentum 20", "Momentum 60", "Momentum 120", "Momentum 252")
        if column in df.columns
    ]
    available_risk = [
        column
        for column in ("Volatility 20", "Drawdown 60", "Rolling Beta 60")
        if column in df.columns
    ]
    available_trend = [
        column
        for column in ("Trend Strength 20", "Trend Strength 60", "Trend Efficiency 20")
        if column in df.columns
    ]
