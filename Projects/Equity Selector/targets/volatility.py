...
def future_volatility(df, horizons=(5, 10, 20, 60), annualize=False):
    ...
    for horizon in horizons:
        df[f"Future Volatility {horizon}"] = (
            returns.shift(-1)[::-1].rolling(horizon).std()[::-1] * multiplier
        )
...
def future_upside_downside_volatility(df, horizons=(10, 20, 60)):
    ...
    for horizon in horizons:
        ...
        complete = df["Close"].shift(-1)[::-1].rolling(horizon).count()[::-1].eq(horizon)
        upside = upside.where(complete)
        downside = downside.where(complete)
...
def future_absolute_return(df, horizons=(5, 20, 60)):
    ...
    for horizon in horizons:
        df[f"Future Mean Absolute Return {horizon}"] = (
            returns.shift(-1)[::-1].rolling(horizon).mean()[::-1]
        )
        df[f"Future Maximum Absolute Return {horizon}"] = (
            returns.shift(-1)[::-1].rolling(horizon).max()[::-1]
        )
