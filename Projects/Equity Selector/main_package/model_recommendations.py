...
def recommend_models_to_fit(
    ...
    for family in families_avaliable:
        ...
        family_search_df = family_search_df.rename(
            columns={"index": "_Search DF Index"}
        )
        ...
        def parse_parameters(value):
            ...
            if not isinstance(parsed_value, dict):
                ...
                raise ValueError(
                    "Parameters must contain a dictionary | "
                    f"family={family!r} | "
                    f"value={value!r}"
                )
        ...
        conflicting_columns = set(
            expanded_parameters.columns
        ).intersection(family_search_df.columns)
        ...
        for parameter in expected_parameter_columns:
            ...
            unique_values = family_search_df[parameter].dropna().unique()
        ...
        current_tpe_max = float(
            family_tpe_scores.max()
        )
        ...
        if not family_tpe_scores.eq(
            current_tpe_max
        ).all():

            ranked_family_search_df = (
                family_search_df
                .assign(
                    _Numeric_TPE_Score=(
                        family_tpe_scores
            ...
            if (
                not best_with_tpe.empty
                and not best_without_tpe.empty
            ):

                best_with_tpe_score = float(
                    best_with_tpe[
                        "Model Selection Score"
                    ].iloc[0]
                )

                best_without_tpe_score = float(
                    best_without_tpe[
                        "Model Selection Score"
                    ].iloc[0]
                )
        ...
        def add_two_sided_midpoints(
            ...
            if (
                current_x >= x
                or parameter
                not in expected_parameter_columns
            ):

                return

            current_config = anchor_row[
                parameter
            ]
            ...
            if (
                ...
                for parameter_value in unique_values:
                    ...
                    if (
                        ...
                        recommendation_midpoint_column.append(
                            parameter
                        )
        ...
        if not family_tpe_scores.eq(0).all():
            ...
            for parameter in expected_parameter_columns:
                ...
                for value in unique_values:
                    ...
                    finite_tpe_scores = [
                        tpe_score
                        for parameter_scores
                        in configuration_tpe_scores.values()
                        for tpe_score
                        in parameter_scores.values()
                        if np.isfinite(tpe_score)
                    ]

                    maximum_finite_tpe_score = (
                        max(finite_tpe_scores)
                        if finite_tpe_scores
                        else 0
                    )
                    ...
                    for parameter, parameter_scores in (
                        configuration_tpe_scores.items()
                    ):

                        for value, tpe_score in (
                            parameter_scores.items()
                        ):

                            if np.isposinf(tpe_score):
            ...
            while randoms_created < num_of_models//4:
                ...
                clone = np.random.choice(models)
                
                parameter = np.random.choice(expected_parameter_columns)
                non_selected_parameters = [
                    p for p in expected_parameter_columns if p != parameter
                ]
                random_parameters = {}
                random_parameters[parameter] = np.random.choice(parameters[parameter])
                for non_selected_parameter in non_selected_parameters:
                    random_parameters[non_selected_parameter] = clone["params"][non_selected_parameter]
        ...
        midpoint_rows = family_search_df[
            family_search_df["Midpoint?"].notna()
        ].copy()
        ...
        if not midpoint_rows.empty:
            ...
            if "Model Selection Score" not in midpoint_rows.columns:
                ...
                raise KeyError(
                    "Model Selection Score is required when "
                    "Midpoint? contains non-missing values."
                )

            midpoint_rows["_Model Selection Score"] = (
                pd.to_numeric(
            ...
            for midpoint_parameter in midpoint_parameters:
                ...
                parameter_rows = (
                    midpoint_rows[
                        midpoint_rows["Midpoint?"].eq(
                            midpoint_parameter
                        )
                    ]
                    .dropna(
                        subset=["_Model Selection Score"]
                    )
                    .sort_values(
                ...
                if len(parameter_rows) >= 2:
                    ...
                    score_difference = abs(
                        best_two_rows.iloc[0][
                            "_Model Selection Score"
                        ]
                        - best_two_rows.iloc[1][
                            "_Model Selection Score"
                        ]
                    )

                    parameter_has_converged = (
        ...
        if current_x < x:
            ...
            for _, normal_anchor_row in (
                family_search_df.iterrows()
            ):
...
def add_recommendation_midpoints(
    ...
    if (
        len(recommended_models)
        != len(recommendation_midpoint_column)
    ):

        raise ValueError(
            "recommended_models and "
            "recommendation_midpoint_column "
    ...
    parsed_summary_parameters = (
        new_summary["Parameters"]
        .apply(parse_parameters)
    )
    ...
    for (
        ...
        matching_recommendation_mask = (
            ...
            & parsed_summary_parameters.apply(
                lambda row_parameters:
                    any(
                        row_parameters
                        == recommended_parameters
                        for recommended_parameters
                        in recommended_parameter_dictionaries
                    )
            )
        )
