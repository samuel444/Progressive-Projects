...
from equity_selector.settings_catalogue import STAGE_KEYS as MODEL_STAGE_KEYS
from horizon_score.settings_catalogue import STAGE_KEYS as STRATEGY_STAGE_KEYS
STAGE_KEYS = {**MODEL_STAGE_KEYS, **STRATEGY_STAGE_KEYS}
...
def test_all_launcher_settings_are_known_and_import_safe():
    ...
    for stage, name in names.items():
        namespace = runpy.run_path(str((root if (root / name).exists() else root.parent / "Equity Model Research") / name))
...
def test_cache_missing_dates_raise_before_download_or_prompt(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="DOWNLOAD_START"):
        __import__("horizon_score.cli", fromlist=["run_stage"]).run_stage("cache", [], settings={"DATA_DIR": str(tmp_path)})
...
def test_horizon_settings_accessor_survives_portfolio_setting_loop(monkeypatch):
    module = importlib.import_module("horizon_score.stages.horizons")
