...
def test_stage_import_has_no_research_io(stage, monkeypatch):
    ...
    module = importlib.import_module(("horizon_score" if stage in {"horizons", "cache", "simulations", "precise"} else "equity_selector") + ".stages." + stage)
...
def test_missing_credentials_fail_before_network(monkeypatch):
    import horizon_score.signals as signals
