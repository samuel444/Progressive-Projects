...
from horizon_score.stages import precise


@pytest.mark.parametrize(
    "fee_args", [["--cost-bps", "1"], ["--trading-fee", "0.0001"], ["--fee-percent", "0.01"]]
)
def test_precise_end_to_end_temporary_databases(tmp_path, fee_args, monkeypatch):
    monkeypatch.setenv("EQUITY_SELECTOR_DATA_DIR", str(tmp_path))
    (tmp_path / "Macro_Regimes.txt").write_text("{}")
