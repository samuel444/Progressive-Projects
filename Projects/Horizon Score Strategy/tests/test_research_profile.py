...
def load(name):
    return runpy.run_path(str((ROOT if (ROOT / name).exists() else ROOT.parent / "Equity Model Research") / name))
...
def test_frozen_signal_group_identity_and_bounded_exhaustive_search():
    ...
    assert precise["PORTFOLIO_GROUP_CONFIGURATIONS"] == final["type_configurations"]
    assert all(group in final["type_configurations"] for group in groups)
    # Horizon screening groups are independent of the later portfolio search.
    horizon_names = [group["Name"] for group in horizon["HORIZON_TEST_TYPE_CONFIGURATIONS"]]
    assert len(horizon_names) == len(set(horizon_names))
    for group in groups:
    ...
    assert horizon["MAX_EXHAUSTIVE_CONFIGURATIONS"] > 0
    assert size <= horizon["MAX_EXHAUSTIVE_CONFIGURATIONS"] or horizon["RANDOM_SCREENS"]
...
def test_cache_launcher_dispatches_selection_dates_without_running_research(monkeypatch):
    import horizon_score.cli

    calls = []
    monkeypatch.setattr(
        horizon_score.cli, "run_stage", lambda stage, **kwargs: calls.append((stage, kwargs))
    )
...
def test_cache_launcher_dispatches_final_dates_without_mutating_selection(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        horizon_score.cli, "run_stage", lambda stage, **kwargs: calls.append(kwargs["settings"])
    )
    source = (
