...
from horizon_score.account import gbp_account_returns
...
def test_preparation_never_overwrites_different_frozen_inputs(tmp_path):
    ...
    for name in ["Final_Test_Results.db", "Selected_Features.txt", "Macro_Regimes.txt", "Top_Horizon_Scores.txt"]:
...
def test_m2_launcher_configures_threads_before_dispatch(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["Run Strategy.py"])
    namespace = runpy.run_path(str(root / "Run Strategy.py"))
    called = []
    ...
    namespace.update(__name__="__main__", __file__=str(root / "Run Strategy.py"))
    exec(
        compile((root / "Run Strategy.py").read_text(), str(root / "Run Strategy.py"), "exec"),
        namespace,
    )
    assert called == [(str(root / namespace["STAGES"][namespace["SETTINGS"]["stage"]]), str(namespace["SETTINGS"]["math_threads"]), str(namespace["SETTINGS"]["joblib_cpu_limit"]))]
