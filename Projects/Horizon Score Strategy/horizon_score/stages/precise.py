...
from horizon_score.scoring import build_score_stocks_with_direction
from horizon_score.scoring import apply_horizon_signal_refresh
from horizon_score.scoring import add_horizon_scores
from horizon_score.scoring import add_type_scores
...
def fe_sharpe(r, annual=True):
    from horizon_score.evaluation import sharpe
    return sharpe(r, FE_DAYS, FE_RF_ANNUAL, annual)


def fe_psr(r, benchmark=0.0):
    from horizon_score.evaluation import probabilistic_sharpe
    return probabilistic_sharpe(r, benchmark, FE_DAYS, FE_RF_ANNUAL)
...
def fe_costs(frame):
    from horizon_score.evaluation import portfolio_daily, cost_metrics
    daily = portfolio_daily(frame.drop(columns=["Date", "Return"]).astype(float))
    costs = cost_metrics(frame.Return, daily["Traded Notional"], FE_TRADING_FEE, FE_DAYS, FE_RF_ANNUAL)
    return {"Annual Turnover": daily.Turnover.mean()*FE_DAYS,
            "Sharpe at Realistic Costs": costs["Sharpe After Costs"],
            "Sharpe at 2x Costs": costs["Sharpe at 2x Costs"],
            "Break-Even Cost (bps)": costs["Break-Even Cost bps"],
            "Break-Even Cost (%)": costs["Break-Even Cost bps"]/100}, daily["Traded Notional"].to_numpy()


def fe_es(r):
    from horizon_score.evaluation import expected_shortfall
    return expected_shortfall(r)
...
def main(argv=None):
    ...
    regimes = load_macro_regimes(research_input("Macro_Regimes.txt", args.data_dir))
    for number, (sid, detail) in enumerate(details.items()):
        detail["Post Backtest Evaluation"] = evaluate_backtest(
            detail["Daily Results"], regimes, benchmark,
            trading_cost=FE_TRADING_FEE, annualisation=FE_DAYS, risk_free_rate=FE_RF_ANNUAL,
            output_dir=output_dir / f"Strategy_{number}_{sid}",
        )
