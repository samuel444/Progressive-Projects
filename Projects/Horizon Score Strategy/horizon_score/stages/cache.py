from equity_selector.settings import setting as get_setting, callback, required_date

"""cache: original sequential research stage; shared logic is in equity_selector."""

from horizon_score.config import data_root, research_input
...
def run():
    global \
        ...
        horizon, \
        horizon_key, \
        individual_stock_feature_columns, \
        invalid_targets, \
        itertools, \
        logger, \
        logging, \
    ...
    from equity_selector.artifacts import load_models_and_predictions

    logging.basicConfig(
        level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s", datefmt="%H:%M:%S"
    )
    logger = logging.getLogger(__name__)
    MARKET_TICKER = get_setting("MARKET_TICKER", "^GSPC")
    FEATURE_WARMUP_YEARS = get_setting("FEATURE_WARMUP_YEARS", 3)
    TARGET_LOOKAHEAD_DAYS = get_setting("TARGET_LOOKAHEAD_DAYS", 400)
    DEFAULT_TOKENS = get_setting(
        "DEFAULT_TOKENS",
        [
            "AAPL",
            "MSFT",
    ...
    print("=" * 70)
    tokens = get_setting("TOKENS", DEFAULT_TOKENS)
    start_date = required_date("DOWNLOAD_START")
    end_date = required_date("DOWNLOAD_END")
    TRAIN_END = required_date("TRAIN_END")
    BACKTEST_START = required_date("BACKTEST_START")
    BACKTEST_END = required_date("BACKTEST_END")
    if end_date <= start_date:
        raise ValueError("End date must be after start date.")
    download_start = start_date
    download_end = end_date
    symbols = list(dict.fromkeys(tokens + [MARKET_TICKER]))
    logger.info(
    ...
    downloaded_symbols = set(raw_download.columns.get_level_values(0))
    if MARKET_TICKER not in downloaded_symbols:
        raise ValueError(f"Market benchmark {MARKET_TICKER} was not returned by yfinance.")
    raw_market = raw_download[MARKET_TICKER].copy().dropna(how="all")
    raw_market.index = pd.to_datetime(raw_market.index)
    raw_market.index.name = "Date"

    def run_step(dataframe, step_name, function, panel_name):
    ...
    run_step = callback("run_step", run_step)

    def add_all_individual_features(
        dataframe, benchmark_df, panel_name, include_market_comparison=True
    ):
        dataframe = dataframe.copy()
        dataframe["Return"] = dataframe["Close"].pct_change()
        feature_steps = [
            ("return features", lambda x: all_return_features(x)),
            ("momentum features", lambda x: all_momentum_features(x)),
            ("volatility features", lambda x: all_volatility_features(x)),
            ("range-volatility features", lambda x: all_range_volatility_features(x)),
            ("trend features", lambda x: all_trend_features(x)),
            ("moving-average features", lambda x: all_moving_average_features(x)),
            ("drawdown features", lambda x: all_drawdown_features(x)),
            ("distribution features", lambda x: all_distribution_features(x)),
            ("tail-risk features", lambda x: all_tail_risk_features(x)),
            ("volume features", lambda x: all_volume_features(x)),
            ("liquidity features", lambda x: all_liquidity_features(x)),
            ("OHLC features", lambda x: all_ohlc_features(x)),
        ]
            ...
            feature_steps.extend(
                [
                    (
                        ...
                        lambda x: all_market_relative_features(x, market_df=benchmark_df),
                    ),
                    ("beta features", lambda x: all_beta_features(x, market_df=benchmark_df)),
                    (
                        "correlation features",
                        lambda x: all_correlation_features(x, market_df=benchmark_df),
                    ),
                    (
                        "residual features",
                        lambda x: all_residual_features(x, market_df=benchmark_df),
                    ),
                ]
            )
        feature_steps.extend(
            [
                ("technical features", lambda x: all_technical_features(x)),
                ("regime features", lambda x: all_regime_features(x)),
                ("interaction features", lambda x: all_interaction_features(x)),
                ("composite features", lambda x: all_composite_features(x)),
                ("experimental features", lambda x: all_experimental_features(x)),
            ]
        )
        for step_name, feature_function in feature_steps:
            dataframe = run_step(dataframe, step_name, feature_function, panel_name)
        return dataframe

    add_all_individual_features = callback(
        "add_all_individual_features", add_all_individual_features
    )

    def create_market_breadth_and_dispersion(raw_stock_frames):
        if len(raw_stock_frames) <= 1:
            logger.warning("Breadth and dispersion features require more than one usable stock.")
            return pd.DataFrame(index=raw_market.index)
        wide_parts = []
        for ticker, stock_df in raw_stock_frames.items():
            part = stock_df[["Open", "High", "Low", "Close", "Volume"]].copy()
            part["Return"] = part["Close"].pct_change()
            part.columns = pd.MultiIndex.from_product([part.columns, [ticker]])
        ...
        market_feature_columns = [
            column for column in wide.columns if column not in original_columns
        ]
        market_features = wide[market_feature_columns].copy()
        market_features.columns = [
            column[0] if isinstance(column, tuple) else column for column in market_features.columns
        ]
        market_features = market_features.loc[:, ~market_features.columns.duplicated()]
        return market_features

    create_market_breadth_and_dispersion = callback(
        "create_market_breadth_and_dispersion", create_market_breadth_and_dispersion
    )

    def add_all_time_series_targets(dataframe, benchmark_df, panel_name):
        target_steps = [
            ("return targets", lambda x: all_return_targets(x, benchmark_df=benchmark_df)),
            ("volatility targets", lambda x: all_volatility_targets(x)),
            ("direction targets", lambda x: all_direction_targets(x)),
            ("barrier targets", lambda x: all_barrier_targets(x)),
            ("excursion targets", lambda x: all_excursion_targets(x)),
            ("drawdown targets", lambda x: all_drawdown_targets(x)),
            ("risk-adjusted targets", lambda x: all_risk_adjusted_targets(x)),
        ]
        for step_name, target_function in target_steps:
            dataframe = run_step(dataframe, step_name, target_function, panel_name)
        return dataframe

    add_all_time_series_targets = callback(
        "add_all_time_series_targets", add_all_time_series_targets
    )

    raw_stock_frames = {}
    for ticker in tokens:
        if ticker not in downloaded_symbols:
            logger.warning("%s skipped because it was not returned by yfinance.", ticker)
            continue
        ticker_df = raw_download[ticker].copy().dropna(how="all")
        ticker_df.index = pd.to_datetime(ticker_df.index)
        ticker_df.index.name = "Date"
        if ticker_df.empty:
            logger.warning("%s skipped because it has no observations.", ticker)
            continue
    ...
    market_level_features = create_market_breadth_and_dispersion(raw_stock_frames)
    logger.info("S&P500 | beginning all targets")
    market = add_all_time_series_targets(
        dataframe=market, benchmark_df=raw_market, panel_name="MARKET"
    )
    logger.info(
        "MARKET | targets complete | rows=%d | columns=%d", len(market), len(market.columns)
    )
    ...
    for ticker, raw_stock in raw_stock_frames.items():
        logger.info("STOCKS | %s | beginning all features", ticker)
        ...
        if individual_stock_feature_columns is None:
            individual_stock_feature_columns = set(stock.columns)
        logger.info("STOCKS | %s | beginning all targets", ticker)
        stock = add_all_time_series_targets(
            dataframe=stock, benchmark_df=raw_market, panel_name=f"STOCKS | {ticker}"
        )
        stock["Ticker"] = ticker
        stock["Date"] = stock.index
        stock_parts.append(stock.reset_index(drop=True))
    stocks = pd.concat(stock_parts, ignore_index=True, sort=False)
    logger.info(
    ...
    cross_sectional_columns = [
        ...
        and pd.api.types.is_numeric_dtype(stocks[column])
    ]
    if len(raw_stock_frames) > 1:
        logger.info(
        ...
        stocks = all_cross_sectional_features(
            stocks, columns=cross_sectional_columns, date_col="Date"
        )
    if len(raw_stock_frames) > 1:
        logger.info("STOCKS | generating all ranking targets")
        stocks = all_ranking_targets(
            stocks, ticker_col="Ticker", date_col="Date", price_col="Close"
        )
    market_feature_frame = market_level_features.reset_index()
    market_feature_frame = market_feature_frame.rename(
        columns={market_feature_frame.columns[0]: "Date"}
    )
    stocks = stocks.merge(
        market_feature_frame,
        on="Date",
    ...
    def finish_dataframe(dataframe, dataframe_name):
        dataframe = dataframe.copy()
        dataframe["Date"] = pd.to_datetime(dataframe["Date"])
        dataframe = dataframe[
            dataframe["Date"].between(start_date, end_date, inclusive="both")
        ].copy()
        numeric_columns = dataframe.select_dtypes(include="number").columns
        dataframe[numeric_columns] = dataframe[numeric_columns].replace([np.inf, -np.inf], np.nan)
    ...
    finish_dataframe = callback("finish_dataframe", finish_dataframe)

    market = finish_dataframe(market, "MARKET")
    stocks = finish_dataframe(stocks, "STOCKS")
    logging.basicConfig(
        level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s", datefmt="%H:%M:%S"
    )
    logger = logging.getLogger(__name__)
    if "stocks" not in globals():
        raise NameError("The stocks dataframe must already exist before this code runs.")
    if "market" not in globals():
        raise NameError("The market dataframe must already exist before this code runs.")
    from equity_selector.macro_regimes import attach_existing_macro
    stocks = attach_existing_macro(stocks.copy(), get_setting("STOCK_TYPE"))
    market = market.copy()
    logger.info(
    ...
    required_stock_columns = {"Date", "Ticker", "Close", "Return"}
    missing_stock_columns = required_stock_columns - set(stocks.columns)
    if missing_stock_columns:
        raise ValueError(
            "stocks is missing required columns: " + ", ".join(sorted(missing_stock_columns))
        )
    required_market_columns = {"Date", "Close", "Return"}
    missing_market_columns = required_market_columns - set(market.columns)
    if missing_market_columns:
        raise ValueError(
            "market is missing required columns: " + ", ".join(sorted(missing_market_columns))
        )
    ...
    market = market.dropna(subset=["Date", "Close"]).sort_values("Date").reset_index(drop=True)
    DATA_DIR = Path(str(data_root()) + "/")
    FINAL_RESULTS_DB = research_input("Final_Test_Results.db", DATA_DIR)
    SELECTED_FEATURES_FILE = research_input("Selected_Features.txt", DATA_DIR)
    ...
    if not available_start <= TRAIN_END < BACKTEST_START <= BACKTEST_END <= available_end:
        raise ValueError(
            "Dates must satisfy:\navailable start <= training end < backtest start <= backtest end <= available end"
        )
    logger.info("Requested training end: %s", TRAIN_END.date())
    logger.info("Backtest period: %s to %s", BACKTEST_START.date(), BACKTEST_END.date())
    ...
    logger.info("Selected feature map loaded | targets=%d", len(selected_features))
    RESULTS_TABLE = f"{STOCK_TYPE} Passed Test Results"
    logger.info("Loading passed model results | %s", RESULTS_TABLE)
    with sqlite3.connect(FINAL_RESULTS_DB) as connection:
        test_results = pd.read_sql_query(f'SELECT * FROM "{RESULTS_TABLE}"', connection)
    logger.info(
    ...
    missing_result_columns = required_result_columns - set(test_results.columns)
    if missing_result_columns:
        raise ValueError(
            "Passed-results table is missing columns: " + ", ".join(sorted(missing_result_columns))
        )
    selected_models_df = test_results.copy()
    selected_models_df = selected_models_df[
        selected_models_df["Target"].astype(str).isin(selected_features)
    ].copy()
    selected_models_df = selected_models_df[
    ...
    selected_models_df = selected_models_df.dropna(
        subset=["Target", "Model", "Quality Score", "Horizon"]
    ).copy()
    ...
    selected_models_df = (
        selected_models_df.sort_values(sort_columns, ascending=sort_ascending)
        .drop_duplicates(subset=["Target"], keep="first")
        .reset_index(drop=True)
    )
    selected_models_df["Statistical Type"] = (
        selected_models_df["Target Type"].astype(str).str.lower().str.strip()
    )
    selected_models_df["Target Type"] = (
        selected_models_df["Portfolio Target Type"].astype(str).str.upper().str.strip()
    )
    ...
    if selected_models_df.empty:
        raise ValueError("No selected production models remain.")
    target_features = {}
    invalid_targets = []
    for _, model_row in selected_models_df.iterrows():
        target = str(model_row["Target"])
        if target not in stocks.columns:
            invalid_targets.append(f"{target}: target column missing")
            continue
        features = list(selected_features.get(target, []))
        missing_features = [feature for feature in features if feature not in stocks.columns]
        if not features:
            invalid_targets.append(f"{target}: no selected features")
            continue
        if missing_features:
            invalid_targets.append(f"{target}: missing features {missing_features}")
            continue
    ...
    def purged_training_end(requested_train_end, backtest_start, horizon, available_dates):
        available_dates = (
            pd.Series(pd.to_datetime(available_dates))
            .drop_duplicates()
            .sort_values()
            .reset_index(drop=True)
        )
        backtest_positions = available_dates.index[available_dates >= pd.Timestamp(backtest_start)]
        if len(backtest_positions) == 0:
            raise ValueError("Backtest start is outside the available stock dates.")
        backtest_position = int(backtest_positions[0])
        cutoff_position = backtest_position - int(horizon) - 1
        if cutoff_position < 0:
            raise ValueError(f"Not enough data to purge a {int(horizon)}-day target.")
        leakage_safe_cutoff = available_dates.iloc[cutoff_position]
        return min(pd.Timestamp(requested_train_end), pd.Timestamp(leakage_safe_cutoff))

    purged_training_end = callback("purged_training_end", purged_training_end)
    ...
    def apply_horizon_signal_refresh(predictions_df, rebalance_multiplier):
        ...
        for group_values, group_indexes in refreshed.groupby(
            ...
            if not horizon.endswith("d"):
                raise ValueError(
                    f"Daily Horizon Key must end in 'd'. Received {horizon!r} for {ticker!r} / {portfolio_type!r}."
                )
    ...
    all_stock_dates = stocks["Date"].drop_duplicates().sort_values().reset_index(drop=True)
    prediction_parts = []
    model_summaries = []
    skipped_model_parts = []
    missing_artifacts = []
    for model_number, (_, model_row) in enumerate(selected_models_df.iterrows(), start=1):
        ...
        horizon = int(float(model_row["Horizon"]))
        ...
        logger.info(
            "[%d/%d] %s | features=%d | requested train end=%s | purged train end=%s | backtest=%s to %s",
            model_number,
            len(selected_models_df),
            target,
            len(features),
        ...
        columns = list(dict.fromkeys(["Date", "Ticker", "Close", "Return", target] + features))
        target_data = stocks[columns].copy()
        train_mask = target_data["Date"] <= actual_train_end
        backtest_mask = target_data["Date"].between(BACKTEST_START, BACKTEST_END, inclusive="both")
        target_data = target_data[train_mask | backtest_mask].copy()
        target_data["Split"] = np.where(
            target_data["Date"] <= actual_train_end, "TRAIN", "BACKTEST"
        )
        one_model_df = pd.DataFrame([model_row.to_dict()])
        try:
            prepared = load_models_and_predictions(
            ...
            missing_artifacts.append(str(error))
            continue
        ...
        model_summaries.append(prepared["model_summary"])
        if not prepared["skipped_models"].empty:
            skipped_model_parts.append(prepared["skipped_models"])
        logger.info(
    ...
    if missing_artifacts:
        raise FileNotFoundError("Run Produce Confirmed Models.py in Equity Model Research, then resume.\n" + "\n".join(missing_artifacts))
    if not prediction_parts:
        raise ValueError("No selected target predictions were generated.")
    predictions_df = pd.concat(prediction_parts, ignore_index=True, sort=False)
    model_summary_df = pd.concat(model_summaries, ignore_index=True, sort=False)
    skipped_models_df = (
        pd.concat(skipped_model_parts, ignore_index=True, sort=False)
    ...
    selected_model_targets = selected_models_df["Target"].astype(str).drop_duplicates().tolist()
    train_df = stocks[stocks["Date"] <= actual_train_end].copy()
    logger.info(
    ...
    train_df[selected_model_targets] = train_df[selected_model_targets].apply(
        pd.to_numeric, errors="coerce"
    )
    target_metrics = (
        train_df[selected_model_targets]
        .agg(["mean", "std"])
    ...
    predictions_df["Quality Score"] = (
        pd.to_numeric(predictions_df["Quality Score"], errors="coerce").clip(0.0, 1.0).fillna(0.0)
    )
    selected_model_targets = selected_models_df["Target"].astype(str).drop_duplicates().tolist()
    available_market_targets = [
        target for target in selected_model_targets if target in market.columns
    ]
    missing_market_targets = [
        target for target in selected_model_targets if target not in market.columns
    ]
    if missing_market_targets:
        logger.warning(
            "Market dataframe does not contain %d selected model targets; they cannot be placed into market_predictions_df: %s",
    ...
    market = market[["Date", "Return"] + available_market_targets].copy()
    market = market[market["Date"].between(BACKTEST_START, BACKTEST_END, inclusive="both")].copy()
    ...
    market_predictions_df = market_predictions_df.dropna(subset=["Date", "Return"]).copy()
    CONTINUOUS_PORTFOLIO_TARGET_TYPES = {
        "ALPHA",
        "RELATIVE_ALPHA",
        "RISK_ADJUSTED_ALPHA",
    ...
    predictions_df = predictions_df.sort_values(["Date", "Ticker", "Target"]).reset_index(drop=True)
    prediction_target_metadata = (
        predictions_df[["Target", "Portfolio Target Type", "Horizon Key", "Quality Score"]]
        .sort_values(["Target", "Quality Score"], ascending=[True, False])
        .drop_duplicates(subset=["Target"], keep="first")
    )
    market_predictions_df = market_predictions_df.drop(
        columns=["Portfolio Target Type", "Horizon Key", "Quality Score"], errors="ignore"
    ).merge(prediction_target_metadata, on="Target", how="inner", validate="many_to_one")
    logger.info(
    ...
    if market_predictions_df["Portfolio Target Type"].isna().any():
        missing_targets = (
            market_predictions_df.loc[
                market_predictions_df["Portfolio Target Type"].isna(), "Target"
            ]
            .unique()
            .tolist()
        )
        raise ValueError(f"Missing Portfolio Target Type for market targets: {missing_targets}")
    market_predictions_df["Signal"] = market_predictions_df.apply(prediction_to_signal, axis=1)
    logger.info(
        "Market signals created | rows=%d | missing=%d",
        len(market_predictions_df),
        int(market_predictions_df["Signal"].isna().sum()),
    )
    market_predictions_df.dropna(subset=["Signal"], inplace=True)
    market_predictions_df.reset_index(drop=True, inplace=True)
    market_predictions_df["Adjusted Signal"] = (
        market_predictions_df["Signal"] * market_predictions_df["Quality Score"]
    )
    market_predictions_df = (
        market_predictions_df[
            [
    ...
    grouped_market_predictions_df = market_predictions_df.groupby(
    ...
    grouped_predictions_df = (
        ...
        .groupby(["Date", "Ticker", "Portfolio Target Type", "Horizon Key"], as_index=False)
        .agg(Return=("Return", "first"), Signal=("Adjusted Signal", "mean"))
    )
    logger.info(
    ...
    market_backtest = (
        market[market["Date"].between(BACKTEST_START, BACKTEST_END, inclusive="both")]
        .copy()
        .sort_values("Date")
        .reset_index(drop=True)
    )
    logger.info(
        "Ready | predictions_df=%d rows | grouped_predictions_df=%d rows | market_predictions_df=%d rows | grouped_market_predictions_df=%d rows | market_backtest=%d rows",
        len(predictions_df),
        len(grouped_predictions_df),
        len(market_predictions_df),
        len(grouped_market_predictions_df),
    ...
    BACKTEST_DATABASE = Path(str(data_root()) + "/Backtest_Database.db")
    with sqlite3.connect(BACKTEST_DATABASE) as connection:
        write_frame(
            grouped_market_predictions_df, "Market", connection, if_exists="replace", index=False
        )
        write_frame(grouped_predictions_df, "Stocks", connection, if_exists="replace", index=False)
    logger.info(
        "Saved backtest data | database=%s | market rows=%d | stocks rows=%d",
        BACKTEST_DATABASE,
        len(grouped_market_predictions_df),
        len(grouped_predictions_df),
    )
