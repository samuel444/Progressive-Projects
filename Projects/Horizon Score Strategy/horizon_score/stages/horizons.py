...
def run():
    ...
    from horizon_score.backtesting import (
        benchmark_metrics,
        run_portfolio_backtest_from_predictions,
    )
    ...
    FEATURE_DATABASE = model_data_root() / "Features_Targets_Data.db"
    ...
    logger.info(
        "Loading confirmed models for internal TRAIN and generating Horizon-validation predictions"
    )
    prediction_parts = []
    missing_artifacts = []
    for model_number, (_, model_row) in enumerate(selected_models_df.iterrows(), start=1):
        ...
        logger.info(
            "[%d/%d] %s | loading with %d selected features",
            model_number,
            len(selected_models_df),
            target,
            len(features),
