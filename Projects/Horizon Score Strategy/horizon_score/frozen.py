...
from equity_selector.database_audit import digest
    ...
    *,
    ...
    for number, (_, row) in enumerate(selected.iterrows(), 1):
        ...
        if regimes is not None:
            from .evaluation import evaluate_backtest, portfolio_daily
            if market is None:
                raise ValueError("A benchmark is required for comprehensive frozen evaluation")
            gross_frame = frame.copy()
            traded = portfolio_daily(frame.drop(columns=["Date", "Return"]))["Traded Notional"]
            fee = trading_fee + (account.get("execution_cost_fraction", 0.0) if account else 0.0)
            gross_frame["Return"] = gross_frame["Return"] + fee * traded.to_numpy()
            evaluate_backtest(gross_frame, regimes, market, trading_cost=fee,
                              annualisation=annualisation, output_dir=out / f"Strategy_{number}_USD")
        if account is not None:
