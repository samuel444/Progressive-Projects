...
def run_stage(stage, argv=None, *, settings=None, callbacks=None):
    ...
    from equity_selector.settings import stage_settings
    from horizon_score.settings_catalogue import STAGE_KEYS, STAGE_CALLBACKS
    ...
    unknown_hooks |= set(settings.get("FUNCTION_KWARGS", {}) or {}) - set(
    ...
    with ExitStack() as stack:
        ...
        original_dir = os.environ.get("HORIZON_SCORE_DATA_DIR")

        def restore_directory():
            if original_dir is None:
                os.environ.pop("HORIZON_SCORE_DATA_DIR", None)
            else:
                os.environ["HORIZON_SCORE_DATA_DIR"] = original_dir

        stack.callback(restore_directory)
        if settings.get("DATA_DIR"):
            os.environ["HORIZON_SCORE_DATA_DIR"] = str(settings["DATA_DIR"])
        logging.basicConfig(
        ...
        logger.info(
            "Completed stage %s | elapsed=%.2fs", stage, perf_counter() - started
        )
        return result
